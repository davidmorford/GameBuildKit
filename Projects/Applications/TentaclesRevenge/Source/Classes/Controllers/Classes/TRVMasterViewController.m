...
@interface TRVMasterViewController ()
	-(void) valueDidChangeForSegmentedControl:(id)sender;
@end

#pragma mark -

@implementation TRVMasterViewController
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == 0) {
		targetURL = @"tentaclesrevenge://settings";
	}
	else if (indexPath.section == 1) {
		if (indexPath.row == 0) {
			targetURL = @"tentaclesrevenge://toolbar";
		}
		else if (indexPath.row == 1) {
			targetURL = @"tentaclesrevenge://navigationbar";
		}
	}
